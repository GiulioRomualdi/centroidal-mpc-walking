...
#ifndef CENTROIDAL_MPC_WALKING_SYSTEM_BLOCK_RUNNER_H
#define CENTROIDAL_MPC_WALKING_SYSTEM_BLOCK_RUNNER_H
...
{
    static_assert(std::is_base_of<CentroidalMPCWalking::System::Block<_Block>, _Block>::value,
...
bool BlockRunner<_Block>::initialize(std::weak_ptr<BipedalLocomotion::ParametersHandler::IParametersHandler> handler)
{
    ...
    if (ptr == nullptr)
    {
        BLF::log()->error("{} The parameters handler is not valid.", errorPrefix);
        return false;
    }

    if (!ptr->getParameter("sampling_time", m_dT))
    {
        BLF::log()->error("{} Unable to get the sampling time.", errorPrefix);
        return false;
    }

    if (m_dT <= 0)
    {
        BLF::log()->error("{} The sampling time must be a strictly positive number.", errorPrefix);
        return false;
    }
    ...
    {
        BLF::log()->info("{} enable_telemetry parameter not found. The default parameter will be "
                         "used: "
                         "{}.",
                         errorPrefix,
...
template <class _Block> bool BlockRunner<_Block>::setBlock(std::unique_ptr<_Block> block)
{
    ...
    if (block == nullptr)
    {
        BLF::log()->error("{} The block is not valid.", errorPrefix);
        return false;
    }
...
bool BlockRunner<_Block>::setInputResource(std::shared_ptr<SharedResource<BlockInput>> input)
{
    ...
    if (input == nullptr)
    {
        BLF::log()->error("{} The input is not valid.", logPrefix);
        return false;
    }
...
template <class _Block> std::thread BlockRunner<_Block>::run()
{
    ...
    if (!m_isInitialized)
    {
        BLF::log()->warn("{} The BlockRunner class is not initialized. An invalid thread will be "
                         "returned",
                         logPrefix);
    ...
    {
        BLF::log()->warn("{} The shared resources are not valid. An invalid thread will be "
                         "returned.",
                         logPrefix);
    ...
    if (m_block == nullptr)
    {
        BLF::log()->warn("{} The block is not valid. An invalid thread will be returned.",
                         logPrefix);
    ...
    if (m_isRunning)
    {
        BLF::log()->warn("{} The thread is already running. An invalid thread will be returned.",
                         logPrefix);
        assert(false && "[BlockRunner::run] The thread is already running.");
        return std::thread();
    }
    ...
    auto function = [&]() -> bool {
        while (this->m_isRunning)
        {
            ...
            {
                m_isRunning = false;
                BLF::log()->error("{} Unable to set the input to the block.", logPrefix);
                return false;
            }
            ...
            {
                m_isRunning = false;
                BLF::log()->error("{} Unable to advance the block.", logPrefix);
                return false;
            }
...
#endif // CENTROIDAL_MPC_WALKING_SYSTEM_BLOCK_RUNNER_H
